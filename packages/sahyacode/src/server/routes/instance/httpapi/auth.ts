...
function validateCredential<A, E, R>(
  ...
  return Effect.gen(function* () {
    if (!Flag.SAHYACODE_SERVER_PASSWORD) return yield* effect

    if (credential.username !== (Flag.SAHYACODE_SERVER_USERNAME ?? "opencode")) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    if (Redacted.value(credential.password) !== Flag.SAHYACODE_SERVER_PASSWORD) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    return yield* effect
  })
