...
from sahya_code.approval_runtime import ApprovalRuntime
...
from sahya_code.soul.toolset import KimiToolset, WireExternalTool
from sahya_code.utils.aioqueue import Queue, QueueShutDown
from sahya_code.utils.logging import logger
from sahya_code.utils.signals import install_sigint_handler
from sahya_code.wire import Wire
...
class WireServer:
    ...
    def _approval_runtime(self) -> ApprovalRuntime | None:
        if isinstance(self._soul, SahyaSoul):
    ...
    async def serve(self) -> None:
        ...
        if isinstance(self._soul, SahyaSoul) and self._soul.runtime.root_wire_hub is not None:
    ...
    async def _shutdown(self) -> None:
        ...
        if (
            isinstance(self._soul, SahyaSoul)
    ...
    async def _handle_initialize(
        ...
        toolset = None
        if isinstance(self._soul, SahyaSoul) and isinstance(self._soul.agent.toolset, KimiToolset):
        ...
        from sahya_code.constant import NAME, VERSION
        from sahya_code.hooks.config import HOOK_EVENT_TYPES
        from sahya_code.hooks.engine import WireHookHandle, WireHookSubscription
        from sahya_code.soul import wire_send
        from sahya_code.wire.protocol import WIRE_PROTOCOL_VERSION
        from sahya_code.wire.types import HookResolved, HookTriggered
    ...
    def _sync_ask_user_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from sahya_code.tools.ask_user import NAME as ASK_USER_TOOL_NAME
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from sahya_code.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from sahya_code.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        if not isinstance(self._soul, SahyaSoul):
            return
    ...
    async def _handle_prompt(
        ...
        runtime = self._soul.runtime if isinstance(self._soul, SahyaSoul) else None
        try:
            await run_soul(
                self._soul,
    ...
    async def _handle_steer(
        ...
        if not isinstance(self._soul, SahyaSoul) or not self._is_streaming:
            return JSONRPCErrorResponse(
                id=msg.id,
                error=JSONRPCErrorObject(
                    code=ErrorCodes.INVALID_STATE,
    ...
    async def _handle_replay(
        ...
        wire_file = self._soul.wire_file if isinstance(self._soul, SahyaSoul) else None
