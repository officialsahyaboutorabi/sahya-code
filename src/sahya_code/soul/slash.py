...
import sahya_code.prompts as prompts
from sahya_code import logger
from sahya_code.soul import wire_send
from sahya_code.soul.agent import load_agents_md
from sahya_code.soul.context import Context
from sahya_code.soul.message import system
from sahya_code.utils.export import is_sensitive_file
from sahya_code.utils.path import sanitize_cli_path, shorten_home
from sahya_code.utils.slashcmd import SlashCommandRegistry
from sahya_code.wire.types import StatusUpdate, TextPart

if TYPE_CHECKING:
    from sahya_code.soul.sahyasoul import SahyaSoul

type SoulSlashCmdFunc = Callable[[SahyaSoul, str], None | Awaitable[None]]
"""
A function that runs as a SahyaSoul-level slash command.
...
@registry.command
async def init(soul: SahyaSoul, args: str):
    """Analyze the codebase and generate an `AGENTS.md` file"""
    from sahya_code.soul.sahyasoul import SahyaSoul

    with tempfile.TemporaryDirectory() as temp_dir:
        tmp_context = Context(file_backend=Path(temp_dir) / "context.jsonl")
        tmp_soul = SahyaSoul(soul.agent, context=tmp_context)
...
@registry.command
async def compact(soul: SahyaSoul, args: str):
...
async def clear(soul: SahyaSoul, args: str):
    """Clear the context"""
...
@registry.command
async def yolo(soul: SahyaSoul, args: str):
...
@registry.command
async def plan(soul: SahyaSoul, args: str):
...
async def add_dir(soul: SahyaSoul, args: str):
    """Add a directory to the workspace. Usage: /add-dir <path>. Run without args to list added dirs"""  # noqa: E501
    from kaos.path import KaosPath

    from sahya_code.utils.path import is_within_directory, list_directory
...
@registry.command
async def export(soul: SahyaSoul, args: str):
    """Export current session context to a markdown file"""
    from sahya_code.utils.export import perform_export
...
async def import_context(soul: SahyaSoul, args: str):
    """Import context from a file or session ID"""
    from sahya_code.utils.export import perform_import
