...
from sahya_code.web.runner.messages import new_session_status_message
...
class SessionProcess:
    """Manages a single session's SahyaCode subprocess.

    Handles:
    - Starting/stopping the subprocess
    - Reading from stdout (wire messages from SahyaCode)
    - Writing to stdin (user input to SahyaCode)
    ...
    async def start(
        ...
        *,
        ...
        """Start the SahyaCode subprocess."""
    ...
    async def _encode_uploaded_files(self) -> AsyncGenerator[ContentPart]:
        ...
        uploads_dir = session.sahya_code_session.dir / "uploads"
