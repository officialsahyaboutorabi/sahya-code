...
from sahya_code.ui.shell.mcp_status import render_mcp_console
from sahya_code.ui.shell.task_browser import TaskBrowserApp
from sahya_code.utils.changelog import CHANGELOG
from sahya_code.utils.datetime import format_relative_time
from sahya_code.utils.slashcmd import SlashCommand, SlashCommandRegistry
...
def ensure_sahya_soul(app: Shell) -> SahyaSoul | None:
    if not isinstance(app.soul, SahyaSoul):
        console.print("[red]SahyaSoul required[/red]")
        return None
    return app.soul
...
def help(app: Shell, args: str):
    ...
    from sahya_code.utils.rich.columns import BulletColumns
...
def version(app: Shell, args: str):
    ...
    from sahya_code.constant import VERSION

    console.print(f"sahya, version {VERSION}")


@registry.command
async def model(app: Shell, args: str):
    """Switch LLM model or thinking mode"""
    from sahya_code.llm import derive_model_capabilities
...
async def editor(app: Shell, args: str):
    ...
    from sahya_code.utils.editor import get_editor_command
...
async def feedback(app: Shell, args: str):
    ...
    from sahya_code.auth.platforms import get_platform_by_id, managed_provider_key
    from sahya_code.constant import VERSION
    from sahya_code.ui.shell.oauth import current_model_key
    from sahya_code.utils.aiohttp import new_client_session

    ISSUE_URL = "https://github.com/sahyaboutorabi/sahya-code/issues"
    ...
    sahya_platform = get_platform_by_id(KIMI_CODE_PLATFORM_ID)
    if sahya_platform is None:
        _fallback_to_issues()
        return
    ...
    feedback_url = f"{sahya_platform.base_url.rstrip('/')}/feedback"
...
async def clear(app: Shell, args: str):
    """Clear the context"""
    if ensure_sahya_soul(app) is None:
        return
