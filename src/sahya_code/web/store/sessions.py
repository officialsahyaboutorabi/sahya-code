...
from sahya_code.metadata import WorkDirMeta, load_metadata
from sahya_code.session import Session as SahyaCodeSession
from sahya_code.web.models import Session
...
class JointSession(Session):
    ...
    sahya_code_session: SahyaCodeSession = Field(exclude=True)
...
def _build_kimi_session(entry: SessionIndexEntry) -> SahyaCodeSession:
    from kaos.path import KaosPath

    from sahya_code.session_state import load_session_state

    return SahyaCodeSession(
...
def _build_joint_session(entry: SessionIndexEntry) -> JointSession:
    ...
    return JointSession(
        ...
        sahya_code_session=kimi_session,
        archived=archived,
    )
