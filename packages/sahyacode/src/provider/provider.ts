  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `sahyacode/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
        ...
        Effect.gen(function* () {
          ...
          for (const [id, provider] of Object.entries(providers)) {
            ...
            for (const [modelID, model] of Object.entries(provider.models)) {
              ...
              if (model.status === "alpha" && !Flag.SAHYACODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
