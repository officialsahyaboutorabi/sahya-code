...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run sahyacode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for sahyacode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found sahyacode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["sahyacode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting sahyacode...")
        UI.println()

        const sahyacodeArgs = sessionId ? ["-s", sessionId] : []
        const sahyacodeProcess = Process.spawn(["sahyacode", ...sahyacodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await sahyacodeProcess.exited
        if (code !== 0) throw new Error(`sahyacode exited with code ${code}`)
      },
    })
  },
})
