...
from sahya_code.ui.theme import get_mcp_prompt_colors
from sahya_code.utils.rich.columns import BulletColumns
from sahya_code.wire.types import MCPServerSnapshot, MCPStatusSnapshot
...
def render_mcp_console(snapshot: MCPStatusSnapshot) -> RenderableType:
    ...
    for server in snapshot.servers:
        ...
        if server.status == "unauthorized":
            server_text += f" [grey50](unauthorized - run: sahya mcp auth {server.name})[/grey50]"
...
def _prompt_server_detail(server: MCPServerSnapshot) -> str:
    if server.status == "unauthorized":
        return f" (unauthorized - run: sahya mcp auth {server.name})"
