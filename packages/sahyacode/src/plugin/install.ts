...
export type PatchDeps = {
  ...
  files: (dir: string, name: "opencode" | "sahyacode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".sahyacode")
}

function patchName(kind: Kind): "sahyacode" | "tui" {
  if (kind === "server") return "sahyacode"
  return "tui"
}
