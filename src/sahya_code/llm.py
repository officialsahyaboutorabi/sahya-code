...
if TYPE_CHECKING:
    ...
    from sahya_code.config import Config, LLMModel, LLMProvider
...
def augment_provider_with_env_vars(provider: LLMProvider, model: LLMModel) -> dict[str, str]:
    ...
    match provider.type:
        ...
        case "openai_legacy" | "openai_responses":
            # Sahya Code environment variables take precedence
            if base_url := os.getenv("SAHYA_BASE_URL"):
                provider.base_url = base_url
                applied["SAHYA_BASE_URL"] = base_url
            elif base_url := os.getenv("OPENAI_BASE_URL"):
                provider.base_url = base_url
            if api_key := os.getenv("SAHYA_API_KEY"):
                provider.api_key = SecretStr(api_key)
                applied["SAHYA_API_KEY"] = "******"
