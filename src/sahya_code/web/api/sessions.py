...
from sahya_code.metadata import load_metadata, save_metadata
from sahya_code.session import Session as SahyaCodeSession
from sahya_code.utils.subprocess_env import get_clean_env
from sahya_code.web.auth import is_origin_allowed, is_private_ip, verify_token
from sahya_code.web.models import (
...
from sahya_code.web.runner.messages import new_session_status_message, send_history_complete
from sahya_code.web.runner.process import SahyaCodeRunner
from sahya_code.web.store.sessions import (
    JointSession,
    SessionMetadata,
...
from sahya_code.wire.jsonrpc import (
    ErrorCodes,
    JSONRPCErrorObject,
...
from sahya_code.wire.serde import deserialize_wire_message
from sahya_code.wire.types import is_request
...
def get_runner(req: Request) -> SahyaCodeRunner:
    """Get the SahyaCodeRunner from the FastAPI app state."""
    return req.app.state.runner


def get_runner_ws(ws: WebSocket) -> SahyaCodeRunner:
    """Get the SahyaCodeRunner from the FastAPI app state (for WebSocket routes)."""
    return ws.app.state.runner


def get_editable_session(
    session_id: UUID,
    runner: SahyaCodeRunner,
) -> JointSession:
...
async def list_sessions(
    runner: SahyaCodeRunner = Depends(get_runner),
    limit: int = 100,
    offset: int = 0,
    q: str | None = None,
...
async def create_session(request: CreateSessionRequest | None = None) -> Session:
    ...
    sahya_code_session = await SahyaCodeSession.create(work_dir=work_dir)
    context_file = sahya_code_session.dir / "context.jsonl"
    invalidate_sessions_cache()
    invalidate_work_dirs_cache()
    return Session(
        session_id=UUID(sahya_code_session.id),
        title=sahya_code_session.title,
        ...
        session_dir=str(sahya_code_session.dir),
    )
...
async def upload_session_file(
    ...
    session_dir = session.sahya_code_session.dir
...
async def get_session_upload_file(
    ...
    uploads_dir = (session.sahya_code_session.dir / "uploads").resolve()
...
async def get_session_file(
    ...
    work_dir = Path(str(session.sahya_code_session.work_dir)).resolve()
...
def _update_last_session_id(session: JointSession) -> None:
    ...
    kimi_session = session.sahya_code_session
...
async def delete_session(session_id: UUID, runner: SahyaCodeRunner = Depends(get_runner)) -> None:
    """Delete a session."""
    session = get_editable_session(session_id, runner)
    session_process = runner.get_session(session_id)
    if session_process is not None:
        await session_process.stop()
    wd_meta = session.sahya_code_session.work_dir_meta
...
async def fork_session(
    ...
    source_dir = source_session.sahya_code_session.dir
    ...
    work_dir = source_session.sahya_code_session.work_dir
    new_session = await SahyaCodeSession.create(work_dir=work_dir)
...
async def generate_session_title(
    ...
    try:
        ...
        from sahya_code.config import load_config
        from sahya_code.llm import create_llm
...
async def session_stream(
    session_id: UUID,
    websocket: WebSocket,
    runner: SahyaCodeRunner = Depends(get_runner_ws),
) -> None:
    ...
    try:
        ...
        try:
            ...
            work_dir = Path(str(session.sahya_code_session.work_dir))
