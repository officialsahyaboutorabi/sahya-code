  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}sahyacode -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
...
function ReasoningPart(props: { last: boolean; part: ReasoningPart; message: AssistantMessage }) {
  ...
  const content = createMemo(() => {
    ...
    // Filter out encrypted/garbled/redacted reasoning content
    if (isHiddenReasoning(text)) {
      return ""
    }
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.SAHYACODE_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
