...
const FILES = [
  "AGENTS.md",
  ...(Flag.SAHYACODE_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.SAHYACODE_CONFIG_DIR) {
    files.push(path.join(Flag.SAHYACODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.SAHYACODE_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.SAHYACODE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no SAHYACODE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.SAHYACODE_CONFIG_DIR, Flag.SAHYACODE_CONFIG_DIR).catch(() => [])
}
