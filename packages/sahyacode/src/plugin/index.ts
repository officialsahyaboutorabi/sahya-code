      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createOpencodeClient({
            ...
            headers: Flag.SAHYACODE_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.SAHYACODE_SERVER_USERNAME ?? "opencode"}:${Flag.SAHYACODE_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.SAHYACODE_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.SAHYACODE_PURE && cfg.plugin_origins?.length) {
