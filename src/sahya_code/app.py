...
from sahya_code.agentspec import DEFAULT_AGENT_FILE
from sahya_code.auth.oauth import OAuthManager
from sahya_code.cli import InputFormat, OutputFormat
from sahya_code.config import Config, LLMModel, LLMProvider, load_config
from sahya_code.llm import augment_provider_with_env_vars, create_llm, model_display_name
from sahya_code.session import Session
from sahya_code.share import get_share_dir
from sahya_code.soul import run_soul
from sahya_code.soul.agent import Runtime, load_agent
...
from sahya_code.utils.logging import logger, redirect_stderr_to_logger
from sahya_code.utils.path import shorten_home
from sahya_code.wire import Wire, WireUISide
from sahya_code.wire.types import ApprovalRequest, ApprovalResponse, ContentPart, WireMessage
...
def enable_logging(debug: bool = False, *, redirect_stderr: bool = True) -> None:
    ...
    logger.enable("sahya_code")
    if debug:
        logger.enable("kosong")
    logger.add(
        get_share_dir() / "logs" / "sahya.log",
...
class SahyaCode:
    ...
    async def create(
        ...
        *,
        ...
        Raises:
            ...
            ConfigError(SahyaCodeException, ValueError): When the configuration is invalid.
            AgentSpecError(SahyaCodeException, ValueError): When the agent specification is invalid.
            SystemPromptTemplateError(SahyaCodeException, ValueError): When the system prompt
                template is invalid.
            InvalidToolError(SahyaCodeException, ValueError): When any tool cannot be loaded.
            MCPConfigError(SahyaCodeException, ValueError): When any MCP configuration is invalid.
            MCPRuntimeError(SahyaCodeException, RuntimeError): When any MCP server cannot be
                connected.
        """
        ...
        try:
            from sahya_code.plugin.manager import (
                collect_host_values,
                get_plugins_dir,
        ...
        soul = SahyaSoul(agent, context=context)
        ...
        return SahyaCode(soul, runtime, env_overrides)
    ...
    @property
    def soul(self) -> SahyaSoul:
        """Get the SahyaSoul instance."""
        return self._soul
    ...
    async def run(
        ...
        Yields:
            WireMessage: The Wire messages from the `SahyaSoul`.
    ...
    async def run_shell(self, command: str | None = None) -> bool:
        ...
        from sahya_code.ui.shell import Shell, WelcomeInfoItem
    ...
    async def run_print(
        ...
        *,
        ...
        from sahya_code.ui.print import Print
    ...
    async def run_acp(self) -> None:
        ...
        from sahya_code.ui.acp import ACP
    ...
    async def run_wire_stdio(self) -> None:
        ...
        from sahya_code.wire.server import WireServer
