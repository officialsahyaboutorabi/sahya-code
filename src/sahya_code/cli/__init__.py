...
cli = typer.Typer(
    ...
    epilog="""\b\
Documentation:        https://github.com/sahyaboutorabi/sahya-code\n""",
...
def sahya(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ~/.local/share/sahya-code/config.toml.",
        ),
    ] = None,
    ...
    from sahya_code.utils.proctitle import init_process_name

    init_process_name("Sahya Code")
    ...
    from sahya_code.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from sahya_code.app import SahyaCode, enable_logging
    from sahya_code.config import Config, load_config_from_string
    from sahya_code.exception import ConfigError
    from sahya_code.hooks import events as hook_events
    from sahya_code.metadata import load_metadata, save_metadata
    from sahya_code.session import Session
    from sahya_code.ui.shell.startup import ShellStartupProgress
    from sahya_code.utils.logging import logger, open_original_stderr, redirect_stderr_to_logger
    ...
    # Redirection is installed later, right before SahyaCode.create(), so that
    ...
    async def _run(session_id: str | None) -> tuple[Session, int]:
        ...
        try:
            ...
            if local_add_dirs:
                from sahya_code.utils.path import is_within_directory
            ...
            # Redirect stderr *before* SahyaCode.create() so that MCP server
            ...
            instance = await SahyaCode.create(
                session,
                config=config,
    ...
    if switch_target in ("web", "vis"):
        ...
        from sahya_code.utils.term import ensure_tty_sane

        ensure_tty_sane()

        if switch_target == "web":
            from sahya_code.web.app import run_web_server

            run_web_server(open_browser=True)
        else:
            from sahya_code.vis.app import run_vis_server
...
def login(
    ...
    """Configure your Sahya Code API key."""
    import asyncio

    from rich.console import Console
    from rich.status import Status

    from sahya_code.auth.oauth import login_kimi_code
...
def logout(
    ...
    from sahya_code.auth.oauth import logout_kimi_code
    from sahya_code.config import load_config
...
def term(
    ...
    """Run Toad TUI backed by Sahya Code ACP server."""
...
@cli.command()
def acp():
    """Run Sahya Code ACP server."""
    from sahya_code.acp import acp_main
...
def background_task_worker(
    ...
    from sahya_code.background import run_background_task_worker
...
def web_worker(session_id: str) -> None:
    ...
    from sahya_code.app import enable_logging
    from sahya_code.web.runner.worker import run_worker
...
if __name__ == "__main__":
    ...
    if "sahya_code.cli" not in sys.modules:
        sys.modules["sahya_code.cli"] = sys.modules[__name__]
