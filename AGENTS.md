...
Sahya Code is a Python CLI agent for software engineering workflows. It supports an interactive
...
- **CLI entry**: `src/sahya_code/cli/__init__.py` (Typer) parses flags (UI mode, agent spec, config, MCP)
  and routes into `SahyaCode` in `src/sahya_code/app.py`.
- **App/runtime setup**: `SahyaCode.create` loads config (`src/sahya_code/config.py`), chooses a
  model/provider (`src/sahya_code/llm.py`), builds a `Runtime` (`src/sahya_code/soul/agent.py`),
  loads an agent spec, restores `Context`, then constructs `SahyaSoul`.
- **Agent specs**: YAML under `src/sahya_code/agents/` loaded by `src/sahya_code/agentspec.py`.
...
- **Tooling**: `src/sahya_code/soul/toolset.py` loads tools by import path, injects dependencies,
  and runs tool calls. Built-in tools live in `src/sahya_code/tools/` (agent, shell, file, web,
  todo, background, dmail, think, plan). MCP tools are loaded via `fastmcp`; CLI management is
  in `src/sahya_code/mcp.py` and stored in the share dir.
- **Subagents**: `LaborMarket` in `src/sahya_code/soul/agent.py` registers builtin subagent types.
  The `Agent` tool (`src/sahya_code/tools/agent/`) creates or resumes subagent instances, while
