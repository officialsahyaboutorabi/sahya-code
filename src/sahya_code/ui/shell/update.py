...
BASE_URL = "https://github.com/sahyaboutorabi/sahya-code/releases"
...
async def _do_update(*, print: bool, check_only: bool) -> UpdateResult:
    ...
    async with new_client_session(timeout=download_timeout) as session:
        ...
        filename = f"sahya-{latest_version}-{target}.tar.gz"
        download_url = f"{BASE_URL}/{latest_version}/{filename}"

        with tempfile.TemporaryDirectory(prefix="sahya-code-") as tmpdir:
            ...
            try:
                ...
                for root, _, files in os.walk(tmpdir):
                    if "sahya" in files:
                        binary_path = os.path.join(root, "sahya")
                        break
                if not binary_path:
