...
Replace runtime Bun shell template-tag usage in `packages/sahyacode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/sahyacode/src` is removed except:
