...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/sahyacode")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/sahyacode")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  ...
  const prompt = [
    ...
    "After resolving the conflicts, run `bun typecheck` in `packages/sahyacode`.",
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  const prompt = [
    ...
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/sahyacode`.",
    "Run `./script/build.ts --single` in `packages/sahyacode`.",
