    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.SAHYACODE_CLIENT) || Flag.SAHYACODE_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.skill,
            tool.patch,
            ...(Flag.SAHYACODE_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            ...(Flag.SAHYACODE_EXPERIMENTAL_PLAN_MODE && Flag.SAHYACODE_CLIENT === "cli" ? [tool.plan] : []),
          ],
          task: tool.task,
          read: tool.read,
        }
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          return input.providerID === ProviderID.opencode || Flag.SAHYACODE_ENABLE_EXA
        }
